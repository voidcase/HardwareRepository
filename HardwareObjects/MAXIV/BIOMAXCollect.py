...
# refactor do_collect
    ...
    def save_image(self):
        ...
        shot_filename = os.path.join(img_dir, "%s_%s_%s.timesnapshot.jpeg" % ( self.current_dc_parameters["fileinfo"]["prefix"], self.current_dc_parameters["fileinfo"]["run_number"], time.strftime('%T')))
    ...
    def store_datacollection_uuid_datacatalog(self):
        ...
        if self.datacatalog_url:
            ...
            logging.getLogger("HWR").error("[HWR] Error sending collection startedinfo to the data catalog: No datacatalog URL specified")
